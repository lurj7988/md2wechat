...
async function syncToWeChat(options) {
  ...
  // 转换为 HTML - 使用 md2wechat 的转换逻辑
  console.log(`📖 正在转换 Markdown...`);
  console.log(`   主题: ${options.theme}, 代码主题: ${options.codeTheme}`);
  const html = render(markdown, { theme: options.theme, codeTheme: options.codeTheme });
  const wrappedHtml = wrapContent(html);
  let wechatHtml = convertToWeChat(wrappedHtml, { theme: options.theme, codeTheme: options.codeTheme });


  // 保存转换后的 HTML 到文件
  const outputFile = options.input.replace(/\.md$/i, '.wechat.html');
  fs.writeFileSync(outputFile, wechatHtml, 'utf8');
  console.log(`💾 HTML 已保存到: ${outputFile}`);
